...
FREE BETA (July 2026): `lib/plans.ts` exports `FREE_BETA = true` — while on, every account resolves to pro limits, billing actions refuse to start, the pricing page shows both plans with no payment CTA, and the sidebar plan badge says "beta". Flip to `false` (founder instruction only) to restore the paid split. `User.plan` is never written by this mechanism.
