...
- `AIRun` — M16D AI run history: one row per "Find ideas" pipeline run (`status` "running" | "completed" | "failed", `inputComplaintCount`, `outputOpportunityCount`, short safe `errorMessage`, `createdAt`, `completedAt`, optional `complaintImportId`). Required `userId`/`projectId` with Cascade relations. `Opportunity.aiRunId` (nullable, `onDelete: SetNull`) links ideas back to their run.
